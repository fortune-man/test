...
ini_set("display_errors", 1);
  ...
  // 첨부파일이 존재한다면 실행
if($_FILES['zenfile']['name']) {
    $baseDownFolder = "../download/";

    // 실제 파일명
    $real_filename = $_FILES['zenfile']['name'];	
    ...
    if( !move_uploaded_file($_FILES["zenfile"]["tmp_name"], $baseDownFolder.$tmp_filename) ) {
    	echo 'upload error';
    }
 ...
 alert("업로드 되었습니다",G5_URL."/server/zenpc_html.php");   
  }
?>
