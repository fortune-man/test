...
function cloudFlareBypass($url){
	...
	curl_setopt_array($ct, Array(
		...
		//CURLOPT_POST => true,
		//CURLOPT_POSTFIELDS => array("url" => $postdata['url'], "date" => $postdata['date'], "football_mode" => $postdata['football_mode']) //schn=csrf'
	));	
	$html = curl_exec($ct);	
    ...
    <?php for ($i=1; $i<=10; $i++) {
		...
		else if($i==3) $ttt = "알림(on/off)";
		else if($i==4) $ttt = "알림주기(초)";		
		else if($i==5) $ttt = "채널id값";
		else if($i==6) {
			$ttt = "html키값";
			if($mb['mb_6']) {
				$json = cloudFlareBypass("https://api.telegram.org/bot".$mb['mb_6']."/getUpdates");
				$chi = json_decode($json,true);
				$chi_cnt = count($chi['result'])-1;
				echo $chi['result'][$chi_cnt]['message']['chat']['id'];
				$tts = ' 마지막 채널 id : '.$chi['result'][$chi_cnt]['message']['chat']['id'].' 내용 : '.$chi['result'][$chi_cnt]['message']['text'].'<a style="color:red;margin;0px 10px;" href="https://api.telegram.org/bot'.$mb['mb_6'].'/sendmessage?chat_id='.$chi['result'][$chi_cnt]['message']['chat']['id'].'&text=findbot" target="_blank"> 테스트 링크</a><a href="https://api.telegram.org/bot'.$mb['mb_6'].'/getUpdates" target="_blank"> api 링크</a> : ';
				for($x=0;$x<$chi_cnt;$x++){
					$chi_row[$x] = $chi['result'][$x]['message']['chat']['id'];
				}
				if(is_array($chi_row)) $ccho = array_unique($chi_row);
			}
			
		}
		else if($i==7) $ttt = "유튜브q";
        else if($i==8) $ttt = "자동알림용(수정금지)";
		else if($i==9) $ttt = "수동검색(초)";
		else if($i==10) $ttt = "시간(수정금지)";
		else {
			$ttt ="여분 필드 ".$i;$tts='';				
		}
	?>
    <tr>
        <th scope="row"><label for="mb_<?php echo $i ?>"><?php echo $ttt;?></label></th>
        <td colspan="3"><input type="text" name="mb_<?php echo $i ?>" value="<?php echo $mb['mb_'.$i] ?>" id="mb_<?php echo $i ?>" class="frm_input" size="30" maxlength="255"><?php if($i==6)echo $tts;?>
        <?php if($ccho && $i==6 ) {
			echo "<br> 기타 채널 id : ";
			print_r($ccho);
		}
