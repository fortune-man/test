...
def realtime_message(number,serial,com_name,usr,setmsg,tomsg,s_type,controle,c_q,nick):
    ...
    try:
        ...
        if get_xpath_xy("채팅 작업","desc","","",number,serial):#설문조사옵션        
            time.sleep(2)
            get_xpath_xy("설문조사 닫기","text","","",number,serial)#설문닫기
            get_xpath_xy("고정된 메세지 닫기","text","","",number,serial)#설문닫기 #2023-08-07
    ...
    chat_x, chat_y = get_xpath_xy("~채팅하기","msgsend","","",number,serial)
                        ...
                        if filter_realtime(serial,server['mode'],server['mode2'],device_path,number) == "on":
                            ...
                            if msg == "viewstart":
                                ...
                                try:
                                    ...
                                    print("/?mode=log&msg="+msg+"&com="+com_name+"&ip="+ip+"&usr="+str(number)+"&c_q="+str(c_q)+"&idnet="+str(idnet))                                    
                                    print(chatinfo)
